...
def replace_leaf(t, old, new):
    ...
    if is_leaf(t) and label(t) == old:
        ...
        new_children = [replace_leaf(child, old, new) for child in branches(t)]
...
def balanced(m):
    ...
    False
    """
    if False in [balanced(end(x)) for x in sides(m) if not is_weight(end(x))]:
    	return False
    if label(sides(m)[0]) * total_weight(end(sides(m)[0])) != label(sides(m)[1]) * total_weight(end(sides(m)[1])):
    	return False
    return True
